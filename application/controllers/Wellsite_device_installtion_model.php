<?php
class Wellsite_device_installtion_model extends CI_Model
{
	...
	public function CheckWell_formula_Exist($well_type)
	{
		$res = $this->db->select("count(id) as total")
			->from('tbl_well_sensor_formula_setup_master')
	...
	{
		return $this->db->select("wf.well_type,wf.component_id,cm.component_name,wf.setup_quantity")
		->from('tbl_well_sensor_formula_setup_master wf')
	...
	{
		$result = $this->db->insert('tbl_site_device_installation',$data);
