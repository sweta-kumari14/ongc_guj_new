...
class FeederMaster extends REST_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('FeederMaster_model');
	}
	...
	public function AddFeeder_post()
	{
		...
		}elseif($this->input->post('c_by',true) == '')
		{
			...
			try 
			{	
				$verifyFeeder = $this->FeederMaster_model->verifyFeederExist($this->input->post('feeder_name',true));
				// print_r($verifyFeeder);die;
				if($verifyFeeder == 0)
				{
					
					$id = $this->FeederMaster_model->getFeederId();
					$data = [];
					$data['id'] = $id[0]['UUID()'];
					...
					$data['status'] = 1;
					$this->FeederMaster_model->saveFeeder($data);
    ...
    public function FeederList_post()
	{
		try {
			...
			$result = $this->FeederMaster_model->Feeder_List($area_id,$id);
	...
	public function FeederList_sitedata_post()
	{
		try {
			...
			$result = $this->FeederMaster_model->Feeder_List_data($site_id);
			$this->response(['status'=>true,'data'=>$result,'msg'=>'successfully Fetched!!','response_code'=>REST_Controller::HTTP_OK]);
		} catch (Exception $e) {
			$this->response(['status'=>false,'data'=>[],'msg'=>'something went wrong!!','response_code'=>REST_Controller::HTTP_INTERNAL_SERVER_ERROR]);
		}
	...
	public function updateFeederData_post()
	{
		...
		}elseif($this->input->post('d_by',true) == '')
		{
			...
			try 
			{	
				$verifyFeeder = $this->FeederMaster_model->verifyFeederExist_OR_not($this->input->post('feeder_name',true),$this->input->post('id',true));
				// print_r($verifyFeeder);die;
				if($verifyFeeder == 0)
				{
					...
					$data['status'] = 1;
					$this->FeederMaster_model->updateFeederData($data,['id'=>$this->input->post('id',true)]);
