                ...
                if ($verify_well_id == 0) {
                    ...
                    if ($well_type == '') {
                        ...
                        if ($well_type == 0) {
                            ...
                            $well_log['apply_datetime'] = (new DateTime($well_log['from_active_date_time']))->modify('+1 day')->setTime(6, 0, 0)->format('Y-m-d H:i:s');
    ...
    public function Delete_Well_Configration_post()
    {
        try{
            ...
            }elseif($this->input->post('d_by',true) == '')
            {
                ...
                $check_currentDateRecord = $this->Well_configuration_model->checkCurrent_Daterecode($this->input->post('well_id',true));

                    
                if($check_currentDateRecord>0)
                {
                    $this->Well_configuration_model->DeleteWell_ConfigrationLogData(['well_id'=>$this->input->post('well_id',true),'date(c_date)'=>date('Y-m-d')]);
                }
                $data = [];
                ...
                if($verify_well == 0)
                {
                   $this->response(['status'=>false,'data'=>[],'msg'=>' id not valid!!','response_code'=>REST_Controller::HTTP_BAD_REQUEST]); 
                    ...
                    if($verify_well_details == 0)
                    {
                       ...
                       $this->Well_configuration_model->DeleteWell_ConfigrationLogData(['well_id'=>$well_id,'date(c_date)'=>date('Y-m-d')]);
                        ...
                        $this->Well_configuration_model->Update_dataWell_Configration(['well_id'=>$well_id,'c_date !='=>date('Y-m-d H:i:s')]);
                         $this->Well_configuration_model->DeleteWell_ConfigrationLogData(['well_id'=>$well_id,'status'=>0,'date(c_date)'=>date('Y-m-d')]);
        ...
        }elseif ($this->input->post('d_by',true) == '') 
        {
            ...
            try {
                ...
                if($well_type == 0)
                {
                    ...
                    $verify_slot = $this->Well_configuration_model->   Verify_Well_configuration_Exist_or_Not($this->input->post('well_id',true),$this->input->post('start_time',true),$this->input->post('stop_time',true));
                    // print_r($verify_slot);die;
                    ...
                    if($verify_slot == 0)
                    {
                        ...
                        if($check_currentDateRecord>0)
                        {
                            $this->Well_configuration_model->DeleteWell_ConfigrationLogData(['well_id'=>$this->input->post('well_id',true),'status'=>1,'date(c_date)'=>date('Y-m-d')]);
                        }
                        ...
                        if ($this->input->post('well_type', true) == $verify[0]['well_type']) 
                        {
                            ...
                            if ($this->input->post('periodic_time') == '') {
                                ...
                                foreach ($periodic_time as $value) 
                                {
                                    ...
                                    if($check_currentDateRecord>0)
                                    {
                                        $this->Well_configuration_model->DeleteWell_ConfigrationLogData(['well_id'=>$this->input->post('well_id',true),'date(c_date)'=>date('Y-m-d'),'well_type'=>0]);
                                    }
                        ...
                        if ($this->input->post('well_type', true) == $verify[0]['well_type']) 
                        {
                            ...
                            if($verify_well == 0)
                            {
                               ...
                               $check_currentDateRecord = $this->Well_configuration_model->checkCurrent_Daterecode($this->input->post('well_id',true));
                                ...
                                if($check_currentDateRecord>0)
                                {
                                    $this->Well_configuration_model->DeleteWell_ConfigrationLogData(['well_id'=>$this->input->post('well_id',true),'date(c_date)'=>date('Y-m-d'),'status'=>0]);
                                }
                            }
