			...
			try{
				...
				{	
					...
					if($checkPrvWell[0]['total'] == 1)
					{
						...
						// $check_well = $this->Well_WiseDeviceshifting_model->getEqu_details($this->input->post('allot_well_id',true));
						// if($check_well>0)
						// {
							...
							$this->response(['status'=>true,'data'=>[],'msg'=>'Successfully device shifted!!','response_code'=>REST_Controller::HTTP_OK]);
						// }else{
						// 	$this->response(['status'=>false,'data'=>[],'msg'=>'Equipment Not set!!','response_code'=>REST_Controller::HTTP_BAD_REQUEST]);
						// }
