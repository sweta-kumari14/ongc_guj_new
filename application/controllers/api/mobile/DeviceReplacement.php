...
class DeviceReplacement extends REST_Controller
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('mobile_model/DeviceReplacement_model');
	}

	public function DeviceReplacement_Data_post()
	{
		...
		}elseif($this->input->post('image',true) == '')
		{
			$this->response(['status'=>false,'data'=>[],'msg'=>'Picture required!!','response_code'=>REST_Controller::HTTP_BAD_REQUEST]);
		}else{
			try 
			{	
				$verify = $this->DeviceReplacement_model->CheckDevie_Exist($this->input->post('old_imei_no',true));
				// print_r($verify);die;

				$replaced_Id = $this->DeviceReplacement_model->getReplacedId();
				if($verify[0]['total'] == 1)
				{
					$id = $this->DeviceReplacement_model->get_Ins_id();
					$image ='';
                    if($this->input->post('image',true)!='')
                    {
		                ...
		                $image = $base64file->du_uploads('album/Replacement/',$image);
                    }
					...
					if($reason_for_replacement == 1)
					{
						...
						}elseif(!preg_match("/^[0-9]{10,20}$/",$new_imei_no))
						{
							...
							$NewDev = $this->DeviceReplacement_model->getDevice_Name($this->input->post('user_id',true),$this->input->post('new_imei_no',true));
							// print_r($NewDev);die;
							if(!empty($NewDev))
							{
								$data = [];
								...
								$data['status'] = 1;
								$this->DeviceReplacement_model->SaveReplecement_Device($data);

								$this->DeviceReplacement_model->UpdateSetup_status(['device_setup_status'=>2,'device_setup_datetime'=>date('Y-m-d H:i:s')],['imei_no'=>$this->input->post('new_imei_no',true)]);
								...
								$replaced['status'] = 1;
								$this->DeviceReplacement_model->SaveReplecementLog_Data($replaced);

								$this->DeviceReplacement_model->Update_Device_and_Sim(['device_name'=>$data['new_device_name'],'imei_no'=>$data['new_imei_no'],'date_of_installation'=>date('Y-m-d H:i:s')],['imei_no'=>$data['old_imei_no']]);
					...
					if($reason_for_replacement == 2)
					{
						...
						$old_sim_no = $this->input->post('old_sim_no',true);
						$new_sim_no = $this->input->post('new_sim_no',true);
						...
						}elseif($this->input->post('old_sim_no',true) == '')
						{
							$this->response(['status'=>false,'data'=>[],'msg'=>'Old Sim Number required!!','response_code'=>REST_Controller::HTTP_BAD_REQUEST]);
						}elseif(!preg_match("/^[0-9]{10}$/",$old_sim_no))
						{
							$this->response(['status'=>false,'data'=>[],'msg'=>'Old Sim Number should be 10 digit allowed!!','response_code'=>REST_Controller::HTTP_BAD_REQUEST]);
						}elseif($this->input->post('new_sim_no',true) == '')
						{
							$this->response(['status'=>false,'data'=>[],'msg'=>'New Sim Number required!!','response_code'=>REST_Controller::HTTP_BAD_REQUEST]);
						}elseif(!preg_match("/^[0-9]{10}$/",$new_sim_no))
						{
							$this->response(['status'=>false,'data'=>[],'msg'=>'New Sim Number should be 10 digit allowed!!','response_code'=>REST_Controller::HTTP_BAD_REQUEST]);
						}else{
								...
								$this->DeviceReplacement_model->Update_Device_and_Sim(['sim_no'=>$data['new_sim_no'],'sim_provider'=>$data['new_sim_provider'],'network_type'=>$data['new_network'],'date_of_installation'=>date('Y-m-d H:i:s')],['imei_no'=>$data['old_imei_no']]);
					...
					if($reason_for_replacement == 3)
					{
						...
						}elseif(!preg_match("/^[0-9]{10}$/",$new_sim_no))
						{
							...
							if(!empty($NewDev))
							{
								...
								$this->DeviceReplacement_model->Update_Device_and_Sim($dataArray,['imei_no'=>$data['old_imei_no']]);
