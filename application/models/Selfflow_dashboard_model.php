...
class Selfflow_dashboard_model extends CI_Model
{
 ...
 public function DashboardWelldetails($company_id, $assets_id, $area_id, $site_id, $user_id, $well_id, $well_type, $user_type, $role_type)
     ...
     public function getSite_for_Map($company_id, $assets_id, $area_id, $user_id, $well_id, $well_type, $site_id, $user_type, $role_type)
    {
        ...
        // Conditional join based on user type and role
        if ($user_type == 3 && $role_type == 6) {
            $this->db->join('tbl_role_wise_user_assign_details ad', 'sd.well_id = ad.well_id', 'left');
            $this->db->where('ad.role_type', $role_type);
        } else {
            $this->db->join('tbl_role_wise_user_assign_details ad', 'w.area_id = ad.area_id', 'left');
        }
