...
class User_complaint_data_model extends CI_Model
{
	...
	public function get_wellself_flow_details($well_id)
	{
		if($well_id!='')
			$this->db->where('well_id',$well_id);
		return $this->db->select('sd.device_name,sd.imei_no,sd.date_time as date_of_installation')
		                ->from('tbl_site_device_installtion_self_flow sd')
		                ->join('tbl_well_master wm','sd.well_id=wm.id','left')
		                ->where(['sd.status'=>1,'sd.well_setup_status'=>1,'wm.status'=>1])
		                ->get()->result_array();
	}
	...
	public function get_complaint_ticket_details($company_id, $area_id, $from_date, $to_date, $ticket_id, $complaint_status, $sort_by)
	{
	    ...
	    $result = $this->db->select("
	            tc.*, 
	            wm.well_name,
	            GROUP_CONCAT(CONCAT(om.user_full_name, ' (', om.userId, ')') ORDER BY tl.c_date ASC) AS user_data
	        ")
	        ->from('tbl_technical_complaint tc')
	        ->join('tbl_technical_complaint_log tl', 'tc.ticket_id=tl.ticket_id', 'left')

	        // ✅ join if well_type = 1
	        ->join('tbl_site_device_installation sd', 
	               'tc.well_id=sd.well_id AND tc.well_type=1', 
	               'left')

	        // ✅ join if well_type = 2
	        ->join('tbl_site_device_installtion_self_flow sf', 
	               'tc.well_id=sf.well_id AND tc.well_type=2', 
	               'left')

	        ->join('tbl_well_master wm', 'tc.well_id=wm.id', 'left')

	        // area join (works for both cases)
	        ->join('tbl_area_master ws', 
	               '(ws.id=sd.area_id OR ws.id=sf.area_id) AND ws.status=1', 
	               'left')
	...
	public function get_complaint_log_data($company_id, $site_id, $from_date, $to_date, $ticket_id, $well_id, $sort_by)
	{
	    ...
	    return $this->db->select("
	            ...
	            tc.device_name, 
	            tc.imei_no, 
	            tc.resolution_description, 
	            tc.complaint_type, 
	            tc.complaint_description, 
	            GROUP_CONCAT(CONCAT(tc.c_date) ORDER BY tc.c_date ASC) AS complaint_date, 
	            GROUP_CONCAT(CONCAT(tc.complaint_status) ORDER BY tc.c_date ASC) AS complaint_status, 
	            wm.well_name
	        ")
	        ->from('tbl_technical_complaint_log tc')
	        ...
	        ->join('tbl_site_device_installtion_self_flow sf', 
	               'tc.well_id=sf.well_id AND tc.well_type=2', 
	               'left')
	        ->join('tbl_well_site_master ws', 
	               '(ws.id=sd.site_id OR ws.id=sf.site_id) AND ws.status=1', 
	               'left')
