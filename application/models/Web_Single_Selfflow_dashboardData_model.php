...
class Web_Single_Selfflow_dashboardData_model extends CI_Model
{
    ...
    public function Single_Well_DeviceData($well_id)
    {
        ...
        $thresholdData = $this->db
            ->select("wt.id,wt.component_id,wt.tag_no,tm.tag_number,wt.upper_value,wt.lower_value,cm.component_name as node_name")
            ->from('tbl_well_threshold_setup_master wt')
            ->join('tbl_component_master cm','wt.component_id=cm.id','left')
            ->join('tbl_tags_number_master tm','tm.id=wt.tag_no','left')
            ->where(['wt.status' => 1, 'wt.well_id' => $well_id])
            ->get()
            ->result_array();
