   ...
   public function getForcasting_data($well_id)
   {
        ...
        if ($current_day == 1) {
            ...
            if ($previous_month_data == 0) {
                $previous_month_data = 12;
                $previous_year_data = $current_year - 1;
            } else {
                $previous_year_data = $current_year;
            }

            $previous_month = sprintf("%02d", $previous_month);
            $previous_month_data = sprintf("%02d", $previous_month_data);

            $from_date = "$previous_year-$previous_month-01 06:00:00";
            $to_date = "$previous_year_data-$previous_month_data-01 05:59:59";
        } else {
            $from_date = "$previous_year-$previous_month-01 06:00:00";
            $to_date = "$current_year-$current_month-01 05:59:59";
        }
