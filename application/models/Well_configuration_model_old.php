...
class Well_configuration_model extends CI_Model
{
	...
	public function get_configration_data($id, $company_id,$user_id,$search)
    {
       ...
       return $this->db->select('wc.id,wc.company_id,wc.well_id,wm.well_name,wc.well_type,wc.assign_date, SUM(distinct wc.running_hours) as running_minutes')
    ...
    public function Verify_Well_configuration_Exist_or_Not($id,$well_id, $start_time, $stop_time)
    {
        if($id!='')
			$this->db->where('id!=',$id);
		 if($well_id!='')
